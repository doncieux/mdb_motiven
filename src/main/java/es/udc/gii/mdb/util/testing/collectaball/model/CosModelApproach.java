 ...
 * @author GII
 */
public class CosModelApproach extends RepresentationApproach {
    ...
    public double[] calculateOutputs(double[] inputs) throws ModelException {
        ...
        double cosAngle1 = ModelsCalculations.getCosAngle();
        double distance1 = ModelsCalculations.getDistance();

        if (distance1 >= maxDistance) {
            cosAngle1 = Math.cos(Math.PI / 2.0);
        }
//        else if (distance1 <= avance) {
//            cosAngle1 = Math.sin(Math.PI / 2.0);
//        }

        cosAngle1 = (cosAngle1 + 1.0) / 2.0;

        return new double[]{cosAngle1};
    }
